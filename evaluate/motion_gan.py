...
#         self.recurrent = nn.GRU(self.input_size, hidden_size, hidden_layer)
...
#         batch_size, motion_length, input_size = motion_sequence.shape
#         # dim (motion_length, num_samples, hidden_size)
#         if hidden_unit is None:
#             # motion_sequence: (batch * length * input_size)
#             velocity = motion_sequence[:, 1:, :] - motion_sequence[:, :-1, :]
#             velocity = torch.cat((torch.zeros((batch_size, 1, input_size), device=motion_sequence.device), velocity), dim=1)

#             # motion_seq + velocity: (batch * len * input*2)
#             motion_sequence = torch.cat((motion_sequence, velocity), dim=2)
