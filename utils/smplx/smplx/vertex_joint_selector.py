...
class VertexJointSelector(nn.Module):
    ...
    def __init__(self,
        ...
        joints_out = (
            'Torso', 'Spine', 'Chest', 'L_Foot', 'R_Foot', 'L_Collar', 'R_Collar', 'Head', 'L_Hand', 'R_Hand'
        )

        joints_in = [joint for joint in orig_joints if joint not in joints_out]

        self.keep_joints_idxs = torch.tensor(
            [orig_joints.index(joint) for joint in joints_in],
            dtype=torch.int64
        )
        ...
        if use_hands:
            self.tip_names = ['index', 'middle', 'ring', 'pinky']
    ...
    def forward(self, vertices, joints):
        joints = torch.index_select(joints, 1, self.keep_joints_idxs.to(joints.device))
